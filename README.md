...
### VSCode (`src/setup-vscode.sh`)

- VSCode 安装（brew cask）+ `code` 命令配置
- 插件分类可选安装（通用 / 前端 / Python / Rust）
- 通用：Prettier, EditorConfig, GitLens, Error Lens, Copilot Chat, Claude Code, IntelliJ 快捷键, SSH 远程开发, Markdown 等
- 前端：ESLint, Tailwind CSS IntelliSense, Pretty TS Errors
- Python：Python 核心（含 Pylance + debugpy）, Ruff
- Rust：rust-analyzer, Even Better TOML, Dependi, CodeLLDB
